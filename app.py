...
def apply_smart_font(paragraph, text, is_bold=False, is_underline=False, force_align=False):
    # If it's a line we want aligned (like the date), we reset all indents
    if force_align:
        paragraph.paragraph_format.left_indent = None
        paragraph.paragraph_format.first_line_indent = None
        paragraph.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.LEFT # Keeps it left-relative to its container
...
with tab1:
    ...
    final_context = {"name": name, "name_capital": name.upper() if name else "", "passport": passport, "dob": dob, "pob": pob, "gender1": g1, "gender2": g2, "gender3": g3, "gender": g1, "date": formatted_date_plain}

    template_file = ""
    if category == "Visa 30 Days Extension": template_file = "visa_30days.docx"; final_context["leave_on"] = st.text_input("Intended Leave Date")
    elif category == "Visa Student": template_file = "visa_student.docx"; final_context.update({"program": st.text_input("Program"), "place_of_study": st.text_input("University"), "location_of_study": st.text_input("Location")})
    elif category == "Visa Employment": template_file = "visa_employment.docx"; final_context.update({"place_of_work": st.text_input("Company"), "location_of_work": st.text_input("Location"), "place_of_issue": st.text_input("Passport Place of Issue"), "country_of_issue": "Nigeria", "date_of_issue": st.text_input("Passport Issue Date"), "passport_expiration": st.text_input("Passport Expiry Date")})
    elif category == "Visa Marriage": template_file = "visa_marriage.docx"
    elif category == "Land Transport": template_file = "land_transport.docx"; final_context["current_address"] = st.text_area("Address"); final_context["purpose"] = st.selectbox("Purpose:", ["registering a driving license as requested", "transferring a vehicle as requested"])
    elif category == "Visa Transfer": template_file = "visa_transfer.docx"; final_context.update({"old_passport": st.text_input("Old Passport No."), "old_passport_expiration": st.text_input("Old Passport Expiry"), "place_of_issue": st.text_input("New Passport Place of Issue"), "date_of_issue": st.text_input("New Passport Issue Date"), "passport_expiration": st.text_input("New Passport Expiry")})

    if st.button("💾 GENERATE DOCUMENT"):
        if name and passport:
            try:
                doc = DocxTemplate(template_file); doc.render(final_context)
                target_stream = io.BytesIO(); doc.save(target_stream); target_stream.seek(0)
                final_doc = Document(target_stream)
                for p in final_doc.paragraphs:
                    if formatted_date_plain in p.text:
                        orig_text = p.text; p.text = ""; apply_smart_font(p, orig_text)
                final_bio = io.BytesIO(); final_doc.save(final_bio)
...
with tab2:
    ...
    with col_a:
        ref_id = st.text_input("Reference No. (Top Line):", value="อีเอ็นบี/ซีเอ็น.07")
        new_issue = st.text_input("New Issue Date (Bottom Line):", value=formatted_date_plain)
    with col_b:
            ...
            with zipfile.ZipFile(zip_buf, "w") as zip_f:
                for f in files:
                    ...
                    for p in list(doc.paragraphs):
                        # 1. Align the Reference Line
                        if ref_id in p.text:
                            # Force alignment for the reference line
                            p.paragraph_format.left_indent = None
                            p.paragraph_format.first_line_indent = None
                            update_next_issue = True
                            continue
                        
                        # 2. Update the Date Line (Must be perfectly aligned with Ref Line)
                        if update_next_issue and len(p.text.strip()) > 0:
                            p.text = ""
                            # force_align=True removes all spacing to ensure vertical stacking
                        ...
                        if "ในเดือน" in p.text:
                            ...
                            orig_p_text = p.text; p.text = ""; apply_smart_font(p, orig_p_text)
                    
                    out = io.BytesIO(); doc.save(out); zip_f.writestr(f.name, out.getvalue())
            st.download_button("📥 Download Updated ZIP", zip_buf.getvalue(), "Updated_Prison_Files.zip")
