...
def apply_smart_font(paragraph, text, is_bold=False, is_underline=False, is_header_block=False):
    ...
    for part in parts:
        ...
        # 2. Font Rules: Thai/Numbers = Angsana New 17, English = TNR 13.5
...
with tab1:
    ...
    if category == "Visa 30 Days Extension": 
        template_file = "visa_30days.docx"; final_context["leave_on"] = st.text_input("Intended Leave Date")
        ...
        template_file = "land_transport.docx"; final_context["current_address"] = st.text_area("Address"); final_context["purpose"] = st.selectbox("Purpose:", ["registering a driving license as requested", "transferring a vehicle as requested"])
    elif category == "Visa Transfer": 
        template_file = "visa_transfer.docx"; final_context.update({"old_passport": st.text_input("Old Passport No."), "old_passport_expiration": st.text_input("Old Passport Expiry Date"), "place_of_issue": st.text_input("New Passport Place of Issue"), "date_of_issue": st.text_input("New Passport Date of Issue"), "passport_expiration": st.text_input("New Passport Expiry Date")})
...
with tab2:
    ...
    with col_a:
        ref_id = st.text_input("Reference No. Search:", value="อีเอ็นบี/ซีเอ็น.07")
            ...
            with zipfile.ZipFile(zip_buf, "w") as zip_f:
                for f in files:
                    ...
                    for p in list(doc.paragraphs):
                        # --- INJECTED DEMAND: GLOBAL REPLACEMENT FOR "แทนอัคร" ON ANY ATTACHED TEXT ---
                        if "แทนอัคร" in p.text:
                            p.text = p.text.replace("แทนอัคร", "แทนเอกอัคร")
